...
<div {{ $attributes->merge(['class' => $baseClasses . ' ' . $sizeClasses]) }} style="{{ $paddingVariables }}" data-strata-table="container">
    <table class="{{ $tableClasses }}" data-strata-table="table">
        {{ $slot }}
    </table>

    @if ($loading)
        <div class="absolute inset-0 bg-background/80 flex items-center justify-center backdrop-blur-xs" data-strata-table="loading">
